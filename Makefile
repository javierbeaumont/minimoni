...
minimoni: $(SRC) $(VENDOR)
	$(CC) $(CFLAGS) -O2 $(SQLITE_FLAGS) $(CIVETWEB_FLAGS) -Ivendor -Isrc -Ibuild -o $@ $^ $(LDFLAGS)

release: embed
	$(CC) $(CFLAGS) -Os -flto $(SQLITE_FLAGS) $(CIVETWEB_FLAGS) \
	  -Ivendor -Isrc -Ibuild -o minimoni $(SRC) $(VENDOR) $(LDFLAGS) -Wl,--gc-sections
	strip minimoni
...
debug: embed
	$(CC) $(CFLAGS) -O0 -g -fsanitize=address,undefined \
	  $(SQLITE_FLAGS) $(CIVETWEB_FLAGS) -Ivendor -Isrc -Ibuild \
	  -o build/minimoni-debug $(SRC) $(VENDOR) $(LDFLAGS_DEBUG)
