...
**`charts`** and **`cards`** control visibility and order. When not set, all metrics are shown
in the default order. Set to `[]` to hide everything. Set to a list to show only those metrics
in the listed order — e.g. `charts = ["memory", "disk"]` shows only those two charts, with
Memory first. Metrics omitted from the list are not included in API responses.
