...
int migrate_preflight(const char *minimoni_exec, const char *db_path, int *out_version)
{
    ...
    /* application_id must be the moni magic. minimoni stamps it on every
     * database since v0.1, so any other value (including 0) means this file
    ...
    if (app_id != MONI_APPLICATION_ID) {
        fprintf(stderr, "migrate: application_id=%ld is not minimoni (expected %ld); refusing\n",
