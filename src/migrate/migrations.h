 ...
 * version: it derives a structural fingerprint of the database (column
 * names, types and indexes) and compares it against the canonical
 * structure of that version, tolerating cosmetic differences such as
 * column order and `CREATE TABLE` whitespace. Returns 0 on match,
 * non-zero on a structural deviation (which causes minimoni-migrate to
 * refuse without writing). The function is responsible for printing the
 * failure reason to stderr. */
typedef struct {
    int from_version;
    int to_version;
